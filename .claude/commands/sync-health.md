...
Sync a quick 1-page summary to the user's Obsidian vault. This is the "I want to glance at my health" view — much shorter than `coach_notes.md`, optimized for skimming on Sunday.

**Vault:** `obsidian-vault`
**Folder:** `10 Projects/HealthOS/Weekly Summaries`
**Filename:** `YYYY-MM-DD — Week NN.md` where YYYY-MM-DD is the sync date and NN is the ISO week number.

Use the `mcp__obsidian__create-note` tool. If the file already exists for today's date, that means the user re-synced — overwrite is OK (the obsidian MCP will reject; if so, append a note clarifying you re-synced).
...
date: YYYY-MM-DD
week: NN
period: <prev sync date> → YYYY-MM-DD
project: HealthOS
tags:
  - health
  - weekly-summary
...
## Links
- **Coach's full deep dive:** [HealthOS coach_notes.md](https://github.com/Savar-G/HealthOS/blob/main/coach_notes.md)
- **Repo:** [github.com/Savar-G/HealthOS](https://github.com/Savar-G/HealthOS)
- **Live dashboard:** localhost:3000 (Insights page)
- **Project page:** [[wiki/HealthOS]]

---
_Generated by `/sync-health` on YYYY-MM-DD. Auto-synced to Obsidian._
```

**Voice:** Tighter than the coach notes. The user is going to scan this in 30 seconds, not read it. One screen, no fluff.
